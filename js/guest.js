...
function decodeShareState(value){const s=value.replace(/-/g,'+').replace(/_/g,'/');const padded=s+'='.repeat((4-s.length%4)%4);const binary=atob(padded),bytes=Uint8Array.from(binary,c=>c.charCodeAt(0));return JSON.parse(new TextDecoder().decode(bytes))}
function filename(name,address){const clean=v=>String(v||'').normalize('NFKC').replace(/[\\/:*?"<>|]/g,'-').replace(/\s+/g,' ').trim().replace(/[. ]+$/,'');return `${clean(name)} - ${clean(address)} - Wedding Invitation.pdf`}
function copyText(text){return navigator.clipboard?.writeText(text).then(()=>true).catch(()=>{try{const ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();const ok=document.execCommand('copy');ta.remove();return ok}catch{return false}})}
function downloadBlob(blob,name){const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},1000)}
function toast(msg){let t=qs('#guestToast');if(!t){t=document.createElement('div');t.id='guestToast';t.className='toast';document.body.appendChild(t)}t.textContent=msg;t.classList.add('show');clearTimeout(t._timer);t._timer=setTimeout(()=>t.classList.remove('show'),2600)}
async function loadFont(){try{const f=new FontFace('NotoDeva',`url(${FONT_URL}) format('truetype')`);await f.load();document.fonts.add(f)}catch(e){console.warn('Font load failed',e)}}
async function renderTextPng(text,opts){
  await document.fonts.ready;const scale=3,pad=10;const c=document.createElement('canvas');const ctx=c.getContext('2d');ctx.font=`${opts.fontWeight} ${opts.fontSize*scale}px NotoDeva`;ctx.textBaseline='alphabetic';const max=opts.maxWidth*scale;const words=String(text||'').split(/\s+/);let lines=[],line='';
  ...
  const lineH=opts.fontSize*scale*1.3;const width=Math.min(max,Math.max(...lines.map(x=>ctx.measureText(x).width),1))+pad*2;const height=lineH*lines.length+pad*2;c.width=Math.ceil(width);c.height=Math.ceil(height);
  ctx.font=`${opts.fontWeight} ${opts.fontSize*scale}px NotoDeva`;ctx.fillStyle=opts.color;ctx.textBaseline='top';ctx.textAlign=opts.align==='center'?'center':opts.align==='right'?'right':'left';const tx=opts.align==='center'?c.width/2:opts.align==='right'?c.width-pad:pad;lines.forEach((ln,i)=>ctx.fillText(ln,tx,pad+i*lineH));return{bytes:await new Promise(r=>c.toBlob(b=>b.arrayBuffer().then(ab=>r(new Uint8Array(ab))),'image/png')),width:c.width/scale,height:c.height/scale}
}
async function buildPdf(name,address,layout){
  const bytes=await fetch(TEMPLATE_URL,{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error('Invitation template could not be loaded.');return r.arrayBuffer()});
  const doc=await PDFLib.PDFDocument.load(bytes,{updateMetadata:false});
  const n=await renderTextPng(name,layout.name),a=await renderTextPng(address,layout.address);const ni=await doc.embedPng(n.bytes),ai=await doc.embedPng(a.bytes);const pg=doc.getPages()[0];
...
async function renderPages(blob){
