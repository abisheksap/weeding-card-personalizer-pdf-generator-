...
function openSocialShare(rec,shareUrl){
  ...
  $('#modalContent').innerHTML=`
    ...
    <div class="share-grid">
      ...
      <button class="share-tile native" id="shareNativeAgain"><span>Phone share</span><small>Share the PDF with phone apps</small></button>
    </div>
...
async function renderInviteOnly(){
  ...
  document.body.innerHTML=`<main class="guest-site">
    <header class="guest-header">
      ...
      <div class="guest-header-actions"><button id="inviteShare" class="btn share">↗ Share PDF</button></div>
    </header>
  ...
  for(let i=1;i<=pdf.numPages;i++){
    ...
    const maxW=Math.min(920,Math.max(300,host.clientWidth-20));
